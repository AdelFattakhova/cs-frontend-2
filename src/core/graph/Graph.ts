...
export default class Graph {
  graph: GraphMatrix | GraphStruct;

  constructor(storage: 'matrix' | 'struct', options?: {}) {
    switch (storage) {
      case ('matrix'):
        this.graph = new GraphMatrix(options);
        break;
      case ('struct'):
        this.graph = new GraphStruct(options);
        break;
    }
  }
}

// TODO: add README.md for Graph
...
// console.log(graph.deleteVertex('Проверить почту'));
